...
export async function transformAndPrint(
    filename: string,
    { identifiers, regexes, oldModuleName, newModuleName }: TransformerOptions,
    makeChanges: boolean
) {
    ...
    try {
        ...
        console.log(
            `Source code of ${filename} before parsing: \n${sourceCode}`
        )
        console.error(e)
    ...
    recast.visit(ast, {
        visitImportDeclaration: function (path) {
            ...
            if (
                node.source.value === oldModuleName &&
    ...
    function isATarget(name?: string): boolean {
        ...
        console.log(`Checking ${name} in against ${regexes?.join(', ')}`)
    ...
    if (hasChanged) {
        ...
        console.log(
            `using prettier config: ${JSON.stringify(
                config
            )} for file ${filename}`
        )
